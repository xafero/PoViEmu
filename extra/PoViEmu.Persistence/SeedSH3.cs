// ReSharper disable InconsistentNaming
// ReSharper disable ClassNeverInstantiated.Global

namespace PoViEmu.Persistence
{
    public class SeedSH3
    {
        public string R0 { get; set; }
        public string R1 { get; set; }
        public string R2 { get; set; }
        public string R3 { get; set; }
        public string R4 { get; set; }
        public string R5 { get; set; }
        public string R6 { get; set; }
        public string R7 { get; set; }
        public string R8 { get; set; }
        public string R9 { get; set; }
        public string R10 { get; set; }
        public string R11 { get; set; }
        public string R12 { get; set; }
        public string R13 { get; set; }
        public string R14 { get; set; }
        public string R15 { get; set; }
        public string R0_b { get; set; }
        public string R1_b { get; set; }
        public string R2_b { get; set; }
        public string R3_b { get; set; }
        public string R4_b { get; set; }
        public string R5_b { get; set; }
        public string R6_b { get; set; }
        public string R7_b { get; set; }
        public string GBR { get; set; }
        public string MACH { get; set; }
        public string MACL { get; set; }
        public string PR { get; set; }
        public string PC { get; set; }
        public string dPC { get; set; }
        public string VBR { get; set; }
        public string SPC { get; set; }
        public string SSR { get; set; }
        public string SR { get; set; }
    }
}