    ...
    internal static class Intel16x8F
    {
        internal static Instruction? Parse(Stream s, byte[] buff, long pos, byte first)
        {
            ...
            switch (second)
            {
                ...
                case 0x06:
                    return new(pos, first, 4, O.pop, [M.word.On(s.NextShort(buff).Box())],   [ second ]  );
                case 0x40:
                    return new(pos, first, 3, O.pop, [M.word.On(R.BX.Plus(R.SI).Minus(s.NextByte(buff)))],   [ second ]  );
                case 0x41:
                    return new(pos, first, 3, O.pop, [M.word.On(R.BX.Plus(R.DI).Minus(s.NextByte(buff)))],   [ second ]  );
                case 0x42:
                    return new(pos, first, 3, O.pop, [M.word.On(R.BP.Plus(R.SI).Plus(s.NextByte(buff)))],   [ second ]  );
                case 0x43:
                    return new(pos, first, 3, O.pop, [M.word.On(R.BP.Plus(R.DI).Plus(s.NextByte(buff)))],   [ second ]  );
                case 0x44:
                ...
                case 0x81:
                    return new(pos, first, 4, O.pop, [M.word.On(R.BX.Plus(R.DI).Signed(s.NextShort(buff)))],   [ second ]  );
                case 0x82:
                    return new(pos, first, 4, O.pop, [M.word.On(R.BP.Plus(R.SI).Signed(s.NextShort(buff)))],   [ second ]  );
                case 0x83:
                    return new(pos, first, 4, O.pop, [M.word.On(R.BP.Plus(R.DI).Signed(s.NextShort(buff)))],   [ second ]  );
                case 0x84:
                    return new(pos, first, 4, O.pop, [M.word.On(R.SI.Signed(s.NextShort(buff)))],   [ second ]  );
                case 0x85:
                    return new(pos, first, 4, O.pop, [M.word.On(R.DI.Signed(s.NextShort(buff)))],   [ second ]  );
                case 0x86:
                    return new(pos, first, 4, O.pop, [M.word.On(R.BP.Signed(s.NextShort(buff)))],   [ second ]  );
                case 0x07:
