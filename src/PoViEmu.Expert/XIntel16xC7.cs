    ...
    internal static class Intel16xC7
    {
        internal static Instruction? Parse(Stream s, byte[] buff, long pos, byte first)
        {
            ...
            switch (second)
            {
                ...
                case 0x45:
                    return new(pos, first, 5, O.mov, [M.word.On(R.DI.Plus(s.NextByte(buff))), s.NextShort(buff)],   [ second ]  );
                case 0x46:
                    return new(pos, first, 5, O.mov, [M.word.On(R.BP.Plus(s.NextByte(buff))), s.NextShort(buff)],   [ second ]  );
                case 0x47:
                    return new(pos, first, 5, O.mov, [M.word.On(R.BX.Plus(s.NextByte(buff))), s.NextShort(buff)],   [ second ]  );
                case 0x80:
