    ...
    internal static class Intel16xC6
    {
        internal static Instruction? Parse(Stream s, byte[] buff, long pos, byte first)
        {
            ...
            switch (second)
            {
                ...
                case 0x45:
                    return new(pos, first, 4, O.mov, [M.@byte.On(R.DI.Plus(s.NextByte(buff)), s.NextByte(buff))],   [ second ]  );
                case 0x46:
                    return new(pos, first, 4, O.mov, [M.@byte.On(R.BP.Plus(s.NextByte(buff)), s.NextByte(buff))],   [ second ]  );
                case 0x47:
                    return new(pos, first, 4, O.mov, [M.@byte.On(R.BX.Plus(s.NextByte(buff)), s.NextByte(buff))],   [ second ]  );
                case 0x80:
